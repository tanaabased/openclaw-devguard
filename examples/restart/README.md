...
node "$GITHUB_WORKSPACE/examples/support/check.mjs" wait-text "$TMPDIR/run.log" "ready        devguard-example" 2 60
find "$DEVGUARD_HOME/projects" -path '*/logs/events.jsonl' -print -quit > "$TMPDIR/log-path"
node "$GITHUB_WORKSPACE/examples/support/check.mjs" assert-restart-log "$(cat "$TMPDIR/log-path")"
